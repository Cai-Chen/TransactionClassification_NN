...
inputData = inputData.InputData('./txn-data_modified.json', './test.json', vocabDir, './category.json')
inputData.getTrainDataAndLabel()

n_input = len(inputData.vocab.vocab)  # Words in vocab
...
for epoch in range(training_epochs):
    batch_xs, batch_ys = inputData.get_train_batch(batch_size)
    sess.run(optimizer, feed_dict={input_tensor: batch_xs, output_tensor: batch_ys, keep_prob: 0.5})
